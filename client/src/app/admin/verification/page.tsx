...
export default function DocumentVerificationPage() {
  ...
  const fetchAudits = async () => {
    setLoading(true);
    try {
      const res = await fetch('http://localhost:5000/api/v1/analytics/documents');
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-border pb-4">
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-gray-400">Compliance & Audit</span>
          </div>
          ...
          <div className="flex items-center bg-surface border border-border rounded-lg p-1">
            <button
              ...
              className={`px-3 py-1.5 rounded text-xs font-mono font-medium transition-all ${
                filterType === 'ALL' ? 'bg-primary text-white font-bold' : 'text-gray-400 hover:text-white'
              }`}
            >
              All Uploads
            </button>
            <button
              ...
              className={`px-3 py-1.5 rounded text-xs font-mono font-medium transition-all ${
                filterType === 'FUEL' ? 'bg-primary text-white font-bold' : 'text-gray-400 hover:text-white'
              }`}
            >
              Fuel Receipts
            </button>
            <button
              ...
              className={`px-3 py-1.5 rounded text-xs font-mono font-medium transition-all ${
                filterType === 'MATERIALS' ? 'bg-primary text-white font-bold' : 'text-gray-400 hover:text-white'
              }`}
            >
              Delivery Notes
            </button>
      ...
      {/* Gallery Grid */}
      {loading ? (
        <div className="p-16 text-center text-gray-400 bg-surface border border-border rounded-xl">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredItems.map((item) => (
            <div key={item.log_id} className="bg-surface border border-border rounded-xl overflow-hidden flex flex-col justify-between">
              <div>
                {/* Proof Media Preview */}
                <div className="relative h-48 bg-neutral-900 border-b border-border/80 group">
                  ...
                  <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                    <button
                      ...
                      className="p-2 bg-primary hover:bg-primary-hover text-white rounded-lg text-xs font-semibold flex items-center gap-1.5"
                    >
                  ...
                  <div className="absolute top-2 left-2">
                    {item.fuel_proof_image ? (
                      <span className="px-2 py-0.5 rounded text-[10px] font-mono font-bold bg-emerald-500/90 text-black">
                ...
                <div className="p-4 space-y-2 text-xs">
                  ...
                  <div className="p-2.5 bg-neutral-900 rounded-lg space-y-1 font-mono text-[11px]">
                    <div className="flex justify-between">
                      <span className="text-muted">Operator:</span>
                      <span className="text-gray-200">{item.operator_name || 'Site Crew'}</span>
                    </div>
                    {item.materials_received && (
                      <div className="flex justify-between">
                        <span className="text-muted">Material Note:</span>
                        <span className="text-primary truncate max-w-[140px]">{item.materials_received}</span>
                      </div>
                    )}
                  </div>
                </div>
              ...
              {/* Approval Stamp */}
              <div className="p-3 bg-neutral-900/60 border-t border-border/50 flex items-center justify-between text-xs">
      ...
      {previewImage && (
        ...
        >
          <div className="max-w-3xl max-h-[85vh] bg-surface p-2 rounded-2xl border border-border shadow-2xl relative">
            <img 
              src={previewImage} 
