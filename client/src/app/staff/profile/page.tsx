...
export default function StaffProfilePage() {
  ...
  return (
    <div className="min-h-screen bg-surface text-ink">
      {/* Top Header */}
      <header className="border-b border-border bg-white sticky top-0 z-40 shadow-subtle">
        <div className="max-w-4xl mx-auto px-4 h-16 flex items-center justify-between">
          <Link 
            href="/staff" 
            className="flex items-center gap-2 text-xs font-mono text-zinc-600 hover:text-primary transition-colors font-semibold"
          >
      ...
      <main className="max-w-4xl mx-auto px-4 py-8 space-y-6">
        ...
        <div className="bg-white border border-border rounded-2xl p-6 relative overflow-hidden shadow-card">
          <div className="flex flex-col sm:flex-row items-center sm:items-start gap-5">
            <div className="w-20 h-20 bg-gradient-to-tr from-primary to-amber-500 rounded-2xl flex items-center justify-center text-white font-heading font-black text-2xl shadow-md">
              {user?.full_name ? user.full_name.slice(0, 2).toUpperCase() : 'OP'}
            </div>

            <div className="flex-1 text-center sm:text-left space-y-1">
              <div className="flex flex-col sm:flex-row sm:items-center gap-2">
                <h1 className="text-2xl font-heading font-black text-ink">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mt-6 pt-6 border-t border-border">
            <div className="p-3.5 bg-surface border border-border rounded-xl">
              ...
              <div className="text-sm font-bold text-ink mt-0.5">Meru County & Mt. Kenya</div>
            </div>
            <div className="p-3.5 bg-surface border border-border rounded-xl">
              <div className="text-[10px] font-mono text-muted uppercase font-semibold">OPERATING PERMIT</div>
              <div className="text-sm font-bold text-emerald-700 mt-0.5">Heavy Plant Class G (Valid)</div>
            </div>
            <div className="p-3.5 bg-surface border border-border rounded-xl">
              <div className="text-[10px] font-mono text-muted uppercase font-semibold">DISPATCH STATUS</div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {[
              ...
              <div key={i} className="p-3.5 bg-surface border border-border rounded-xl flex items-center justify-between hover:border-primary/40 transition-colors">
                ...
                <span className="text-[11px] font-mono text-emerald-700 font-bold bg-emerald-50 px-2 py-0.5 rounded border border-emerald-200">{mach.status}</span>
              </div>
            ))}
          </div>
        </div>
