...
export async function submitStaffLog(req: Request, res: Response) {
  try {
    const {
      staffId,
      equipmentId,
...
export async function getAllStaffLogs(req: Request, res: Response) {
  try {
    const { staffId, equipmentId, startDate, endDate } = req.query;
    let query = `
      SELECT 
        l.id,
        l.staff_id,
